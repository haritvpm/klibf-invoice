    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.products.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('bookfests') ? 'is-invalid' : '' }}" name="bookfests[]" id="bookfests" multiple>
                    @foreach($bookfests as $id => $bookfest)
                        <option value="{{ $id }}" {{ in_array($id, old('bookfests', [])) ? 'selected' : '' }}>{{ $bookfest }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('taxpercent_cgst') ? 'is-invalid' : '' }}" type="number" name="taxpercent_cgst" id="taxpercent_cgst" value="{{ old('taxpercent_cgst', '9') }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('taxpercent_sgst') ? 'is-invalid' : '' }}" type="number" name="taxpercent_sgst" id="taxpercent_sgst" value="{{ old('taxpercent_sgst', '9') }}" step="0.01">
