...
@can('sanctioned_amount_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.sanctioned-amounts.create') }}">
                        ...
                        <tr data-entry-id="{{ $sanctionedAmount->id }}">
                            ...
                            <td>
                                @can('sanctioned_amount_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.sanctioned-amounts.show', $sanctionedAmount->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('sanctioned_amount_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.sanctioned-amounts.edit', $sanctionedAmount->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('sanctioned_amount_delete')
                                    <form action="{{ route('admin.sanctioned-amounts.destroy', $sanctionedAmount->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('sanctioned_amount_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.sanctioned-amounts.massDestroy') }}",
