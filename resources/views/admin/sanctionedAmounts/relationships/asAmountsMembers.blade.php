...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-asAmountsMembers">
                <thead>
                    <tr>
                        <th width="10">
                        ...
                        <th>
                            {{ trans('cruds.member.fields.as_amounts') }}
                        </th>
                        <th>
                            {{ trans('cruds.member.fields.fin_year') }}
                        </th>
                        <th>
                            &nbsp;
                        </th>
                ...
                <tbody>
                    @foreach($members as $key => $member)
                        <tr data-entry-id="{{ $member->id }}">
                            ...
                            <td>
                                @foreach($member->as_amounts as $key => $item)
                                    <span class="badge badge-info">{{ $item->as_amount }}</span>
                                @endforeach
                            </td>
                            <td>
                                {{ $member->fin_year->title ?? '' }}
                            </td>
                            <td>
                                @can('member_show')
...
@endcan
  ...
  let table = $('.datatable-asAmountsMembers:not(.ajaxTable)').DataTable({ buttons: dtButtons })
