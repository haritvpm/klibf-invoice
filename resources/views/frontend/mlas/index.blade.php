        ...
        <div class="col-md-12">
            @can('mla_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.mlas.create') }}">
                            ...
                            <tbody>
                                @foreach($mlas as $key => $mla)
                                    <tr data-entry-id="{{ $mla->id }}">
                                        ...
                                        <td>
                                            @can('mla_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.mlas.show', $mla->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('mla_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.mlas.edit', $mla->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('mla_delete')
                                                <form action="{{ route('frontend.mlas.destroy', $mla->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('mla_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.mlas.massDestroy') }}",
